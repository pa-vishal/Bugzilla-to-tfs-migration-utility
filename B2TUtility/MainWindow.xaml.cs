...
namespace xmlRpcExample
{
    ...
    public partial class MainWindow
    {
        ...
        {
            InitializeComponent();
            //_server = new Server("rdsw-bugzilla.nmr.varianinc.com", "bugzilla");
        ...
        {
            var httpBase = new HttpBaseClass("loginid", "password", "", 0,
                                                      "http://xxx.com/bugzilla/");
            ...
            const string queryStringFirstPart = "http://xxxx.com/bugzilla/show_bug.cgi?id=";
            ...
            while (_batch > 0 && _start <= _batch)
            {
                try
                {
                    foreach (var bugId in Seq(_start, _batch - _start + 1))
                    {
                        ...
                        const string dtdString = "<!DOCTYPE bugzilla SYSTEM \"http://rdsw-bugzilla.nmr.varianinc.com/bugzilla/bugzilla.dtd\">";
                ...
                if (store.Projects[i].Name == projectName)
                {
                    ...
                    for (int j = 0; j < project.WorkItemTypes.Count; j++)
                    {
                        if (project.WorkItemTypes[j].Name == workItemTypeName)
                        {
                            ...
                            newWI.Fields["Bugzilla.ReportedOn"].Value = bug.CreatedDateTime;

                            newWI.Fields["Bugzilla.ModifiedDate"].Value = bug.ChangedDateTime;
                            ...
                            //Note continued:       So use 'Bugzilla.Status' field till we get all the Bugzilla bugs to TFS and then update the custom workitem to show System.Status
                            newWI.Fields["Bugzilla.Status"].Value = bug.BugStatus;

                            newWI.Fields["Bugzilla.Resolution"].Value = bug.Resolution;

                            newWI.Fields["Bugzilla.Priority"].Value = bug.Priority;
                            ...
                            if (bug.DependsOn != 0)
                            {
                                WorkItemCollection wicol =
                                    store.Query("Select [ID] from Issues where (Bugzilla.BugzillaId='" + bug.DependsOn +
                                                "')");
                                if (wicol.Count > 0)
                                {
                            ...
                            {
                                WorkItemCollection wicol =
                                    store.Query("Select [ID] from Issues where (Bugzilla.BugzillaId='" + bug.Blocks +
                                                "')");
                                if (wicol.Count > 0)
                                {
                                    RelatedLink link = new RelatedLink(wicol[0].Id);
                            ...
                            newWI.Fields["Bugzilla.ReportedBy"].Value = bug.ReportedBy;

                            newWI.Fields["Bugzilla.AssignedTo"].Value = bug.AssignedTo;

                            if (bug.CcList != null)
                                newWI.Fields["Bugzilla.Observers"].Value = bug.CcList; // Bugzilla.EmailCCList

                            newWI.Fields["Bugzilla.Estimated_Time"].Value = bug.EstimatedTime;

                            newWI.Fields["Bugzilla.Remaining_Time"].Value = bug.RemainingTime;

                            newWI.Fields["Bugzilla.Actual_Time"].Value = bug.ActualTime;
